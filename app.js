...
function go(view){$$('.view').forEach(x=>x.classList.remove('active'));$('#view-'+view)?.classList.add('active');$$('.nav-item').forEach(x=>x.classList.toggle('active',x.dataset.view===view));if(view==='equipos')renderEquiposStats();if(view==='personal')renderPersonalStats();if(view==='seguimiento')renderSeguimiento();if(view==='mapa')renderMapa();if(view==='buenas')renderBuenas();if(view==='asistente')renderAsistenteIA();if(view==='controles')renderControles();if(view==='revision'&&state.reviewUnlocked)renderReviewQueue();window.scrollTo({top:0,behavior:'smooth'})}
...
function drawChart(id,type,data,key,legend=true){const el=$('#'+id);if(!el)return;if(state.charts[key])state.charts[key].destroy();const labels=Object.keys(data),vals=Object.values(data);state.charts[key]=new Chart(el,{type,data:{labels,datasets:[{label:'Cantidad',data:vals,backgroundColor:colors(labels.length),borderWidth:type==='doughnut'?1:0}]},options:{responsive:true,maintainAspectRatio:false,layout:{padding:{top:type==='bar'?24:4}},plugins:{legend:{display:legend,position:'bottom'},datalabels:{display:true,color:type==='doughnut'?'#fff':'#17212b',anchor:type==='bar'?'end':'center',align:type==='bar'?'top':'center',offset:type==='bar'?4:0,clamp:true,clip:false,font:{weight:'800',size:type==='bar'?12:11},formatter:v=>Number(v)>0?String(v):''}},scales:type==='bar'?{y:{beginAtZero:true,grace:'18%',ticks:{precision:0}}}:{}}})}
...
function normalizeMapDate(v){if(!v)return'';if(v instanceof Date&&!isNaN(v))return `${v.getFullYear()}-${String(v.getMonth()+1).padStart(2,'0')}-${String(v.getDate()).padStart(2,'0')}`;const s=String(v).trim();let m=s.match(/^(\d{4})-(\d{1,2})-(\d{1,2})/);if(m)return `${m[1]}-${m[2].padStart(2,'0')}-${m[3].padStart(2,'0')}`;m=s.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})/);if(m)return `${m[3]}-${m[2].padStart(2,'0')}-${m[1].padStart(2,'0')}`;return s}
function mapDataToday(){const hoy=todayLocalISO();return (state.mapData||[]).filter(x=>normalizeMapDate(x.fecha)===hoy)}
...
function renderMapa(){const emp=$('#mapEmpresa')?.value||'',st=$('#mapEstado')?.value||'',q=norm($('#mapSearch')?.value||'');const arr=mapDataToday().filter(x=>(!emp||x.empresa===emp)&&(!st||x.estado===st)&&(!q||norm(JSON.stringify(x)).includes(q)));const mk=$('#mapMarkers');if(!mk)return;mk.innerHTML='';groupMapBySector(arr).forEach(g=>{const wrap=document.createElement('div');wrap.className='sector-map-marker';wrap.style.left=`${g.x}%`;wrap.style.top=`${g.y}%`;const btn=document.createElement('button');btn.className=`marker ${groupMarkerState(g)}`;btn.type='button';btn.textContent=String(g.items.length);btn.title=`${g.sector}: ${g.items.length} trabajo(s)`;btn.onclick=e=>{e.stopPropagation();showMarkerGroup(g,wrap)};const lab=document.createElement('span');lab.className='sector-map-label';lab.textContent=g.sector;wrap.append(btn,lab);mk.appendChild(wrap)});$('#mapKpis').innerHTML=[['Trabajos',arr.length],['Empresas',new Set(arr.map(x=>x.empresa)).size],['En Ejecución Hoy',arr.filter(x=>norm(x.estado).includes('ejec')).length]].map(k=>`<div class="kpi"><div class="label">${k[0]}</div><div class="value">${k[1]}</div></div>`).join('');const total=$('#mapChartTotal');if(total)total.textContent=`Total hoy: ${arr.length}`;drawChart('chartMapEmpresas','bar',countBy(arr,'empresa'),'mapEmp',false);$('#mapList').innerHTML=tableSimple(arr,['empresa','sector','actividad','estado'],['Empresa','Sector','Actividad','Estado']);requestAnimationFrame(syncMapOverlays)}
...
const IZAJE_CHAT_URL=(cfg.N8N_IZAJE_WEBHOOK||'https://jonathanrl.app.n8n.cloud/webhook/chatbot-sst-docs').trim();
let aiBusy=false;
function renderAsistenteIA(){requestAnimationFrame(()=>{$('#aiQuestion')?.focus()})}
function aiEscape(v=''){return String(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function aiFormatAnswer(raw=''){
  let t=aiEscape(String(raw||'').replace(/\\n/g,'\n')).trim();
  // Markdown simple y seguro para negritas.
  t=t.replace(/\*\*(.+?)\*\*/g,'<strong>$1</strong>');
  const lines=t.split(/\r?\n/);
  let inSource=false, body=[], source=[];
  lines.forEach(line=>{
    const clean=line.trim();
    if(/^FUENTE(S)? DOCUMENTAL(ES)?:?/i.test(clean)){
      inSource=true;
      source.push('<div class="ai-source-title">'+clean.replace(/:$/,'')+'</div>');
      return;
    }
    if(inSource){
      if(clean) source.push('<div class="ai-source-line">'+clean.replace(/^[-•]\s*/,'')+'</div>');
      return;
    }
    if(!clean){body.push('<div class="ai-spacer"></div>');return;}
    if(/^\d+[.)]\s+/.test(clean)) body.push('<div class="ai-number-line">'+clean+'</div>');
    else if(/^[-•]\s+/.test(clean)) body.push('<div class="ai-bullet-line">'+clean.replace(/^[-•]\s*/, '• ')+'</div>');
    else body.push('<div class="ai-text-line">'+clean+'</div>');
  });
  return body.join('')+(source.length?'<div class="ai-source-box">'+source.join('')+'</div>':'');
}
function appendAIMessage(role,text,{loading=false}={}){
  const root=$('#aiMessages');if(!root)return null;
  const row=document.createElement('div');row.className='ai-message '+role+(loading?' loading':'');
  row.innerHTML=role==='assistant'?`<div class="ai-msg-avatar">✦</div><div class="ai-bubble">${loading?'<div class="ai-typing"><i></i><i></i><i></i><span>Consultando documentos…</span></div>':aiFormatAnswer(text)}</div>`:`<div class="ai-bubble">${aiEscape(text)}</div>`;
  root.appendChild(row);root.scrollTop=root.scrollHeight;return row;
}
async function askIzajeAI(question){
  const q=String(question||'').trim();if(!q||aiBusy)return;
  aiBusy=true;const input=$('#aiQuestion'),btn=$('#btnSendAI'),status=$('#aiStatus');
  if(input)input.value='';appendAIMessage('user',q);const wait=appendAIMessage('assistant','',{loading:true});
  if(btn){btn.disabled=true;btn.classList.add('busy')} if(status)status.textContent='Buscando sustento en el repositorio documental…';
  try{
    const r=await fetch(IZAJE_CHAT_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mensaje:q})});
    const text=await r.text();if(!r.ok)throw new Error(`HTTP ${r.status}${text?': '+text.slice(0,180):''}`);
    wait?.remove();appendAIMessage('assistant',text||'No se recibió contenido en la respuesta.');
    if(status)status.textContent='Respuesta generada con consulta al repositorio documental.';
  }catch(e){
    wait?.remove();appendAIMessage('assistant',`No fue posible consultar el asistente en este momento. ${e.message||''}`);
    if(status)status.textContent='No se pudo conectar con el servicio de IA. Intenta nuevamente.';
  }finally{aiBusy=false;if(btn){btn.disabled=false;btn.classList.remove('busy')} input?.focus()}
}
function clearAIChat(){
  const root=$('#aiMessages');if(!root)return;
  root.innerHTML='<div class="ai-message assistant"><div class="ai-msg-avatar">✦</div><div class="ai-bubble"><b>Hola, soy tu Asistente IA de Izaje.</b><p>Puedes preguntarme sobre izajes críticos, competencias, inspecciones, permisos y otros requisitos disponibles en el repositorio.</p></div></div>';
  const st=$('#aiStatus');if(st)st.textContent='Las respuestas se generan a partir de la documentación disponible.';
}
