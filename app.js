...
async function savePersonal(){
  if(!state.peBatch.length)return showModal('Sin registros','<p>Agrega al menos una competencia.</p>');
  const r=await api('savePersonal',{items:state.peBatch});
  state.peBatch=[];
  renderPeBatch();
  await refreshData();

  const vals=Array.isArray(r.validaciones)?r.validaciones:[];
  const resumen=vals.length
    ? `<div style="margin-top:12px">${vals.map(v=>{
        const estado=String(v.estado||'NO EVALUADO').toUpperCase();
        const cls=estado==='VALIDADO'?'success':(estado==='OBSERVADO'?'warning':'');
        return `<div class="notice ${cls}" style="margin-bottom:8px">
          <b>${esc(v.nombre||'Registro')}</b><br>
          Validación automática: <b>${esc(estado)}</b>
          ${v.observacion?`<div style="margin-top:5px">${esc(v.observacion)}</div>`:''}
        </div>`;
      }).join('')}</div>`
    :'';

  showModal('Registro enviado',`<p>Se registraron <b>${r.count}</b> competencias. Los registros quedaron <b>En revisión por UNACEM</b>.</p>${resumen}`);
}
...
function detailReviewActionHtml(type,id,x){
  const st=norm(x?.estadoRevision||'');
  const label=st.includes('aprob')?'Revisar / actualizar por UNACEM':'Revisar por UNACEM';
  return `<div class="detail-review-action">
    <div>
      <b>Revisión UNACEM</b>
      <div class="detail-review-help">Acceso restringido al equipo revisor de UNACEM.</div>
    </div>
    <button class="btn primary" onclick="unlockReviewFromDetail('${type}','${id}')">${label}</button>
  </div>`;
}
window.viewRecord=(type,id)=>{
  const x=(type==='equipo'?state.equipos:state.personal).find(r=>r.id===id);
  if(!x)return;
  showModal(
    type==='equipo'?'Detalle del equipo':'Detalle de competencia',
    `${recordDetailHtml(x)}${detailReviewActionHtml(type,id,x)}`
  );
}
...
window.saveFullEdit=async(type,id)=>{const btn=$('#btnSaveEdit');if(btn.disabled)return;btn.disabled=true;btn.textContent='Procesando...';try{const newFiles=await filesToObjs($('#editFiles'),8);let updates;if(type==='equipo')updates={Estado:$('#editEstado').value,Marca:$('#editMarca').value,Modelo:$('#editModelo').value,Serie:$('#editSerie').value,Capacidad:$('#editCapacidad').value,Lugar:$('#editLugar').value,FechaCertificacion:$('#editFechaCert').value,Vigencia:$('#editVigencia').value,ObservacionEmpresa:$('#editComment').value};else updates={Nombre:$('#editNombre').value,DNI:$('#editDni').value,Capacitacion:$('#editCap').value,FechaCapacitacion:$('#editFecha').value,Vigencia:$('#editVigencia').value,EmpresaCapacitadora:'ISEM',ObservacionEmpresa:$('#editComment').value};await api('updateRecord',{type,id,updates,newFiles});closeModal();await refreshData();showModal('Actualización enviada',type==='equipo'?`<p>La información fue actualizada y el estado operativo del equipo quedó como <b>${esc($('#editEstado')?.value||'')}</b>. El registro volvió a <b>En revisión</b> para mantener la trazabilidad.</p>`:'<p>El registro volvió a estado <b>En revisión</b> con los nuevos documentos adjuntos.</p>')}catch(e){showModal('Error',`<p>${esc(e.message)}</p>`)}finally{if(btn){btn.disabled=false;btn.textContent='Enviar actualización a revisión'}}}
